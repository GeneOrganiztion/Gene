function initOneClassifyManager(){
	...
	jQuery(grid_selector).jqGrid({
		...
		subGridRowExpanded: function (subgridDivId, rowId) {
			...
			$("#" + subgridTableId).jqGrid({
				...
				colModel: [
				        {name:'claPid',index:'claPid', width:80,  hidden: true},
		...
		url: webroot + "classify2/seletcClassify.do",
		mtype: 'post',
		datatype: "json",
	...
	try {
		...
		$.ajax({
			type: "post",
			url: webroot + "classify2/selectAllOneClassify.do",
...
function editTwoClassify(id){
	...
	$.ajax({
		type: "post",
		url: webroot + "classify2/seletcOneTwoClassify.do",
		data: {classifyId: id},
		success: function(msg){
...
function editAndSaveTwoClassify(){
	...
	$.ajax({
		type: "post",
		url: webroot + "classify2/updateTwoClassify.do",
    ...
    Lobibox.confirm({ 
        ...
        callback: function ($this, type, ev) {               //回调函数 
            if (type === 'yes') { 
            	$.ajax({
            		type:"post",
            		url:webroot+"classify2/deleteTwoClassify.do",
            		data:{"twoClassifyIds":id},
            		success:function(msg){
            			//删除成功重新加载jqGrid
            			if(msg.success){
            				alertmsg("success", "删除成功");
...
function saveTwoClassify(){
	...
	$.ajax({
		type:"post",
		url: webroot + "classify2/saveTwoClassify.do",
